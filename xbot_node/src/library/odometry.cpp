/**
 * @file /xbot_node/src/node/odometry.cpp
 *
 * @brief File comment
 *
 * File comment
...
#include "../../include/xbot_node/odometry.hpp"
...
void Odometry::init(ros::NodeHandle& nh, const std::string& name) {
  ...
  ROS_INFO_STREAM("Xbot : Velocity commands timeout: " << cmd_vel_timeout << " seconds [" << name << "].");

  if (!nh.getParam("odom_frame", odom_frame)) {
    ROS_WARN_STREAM("Xbot : no param server setting for odom_frame, using default [" << odom_frame << "][" << name << "].");
  } else {
    ROS_INFO_STREAM("Xbot : using odom_frame [" << odom_frame << "][" << name << "].");
  }

  if (!nh.getParam("base_frame", base_frame)) {
    ROS_WARN_STREAM("Xbot : no param server setting for base_frame, using default [" << base_frame << "][" << name << "].");
  } else {
    ROS_INFO_STREAM("Xbot : using base_frame [" << base_frame << "][" << name << "].");
  }

  if (!nh.getParam("publish_tf", publish_tf)) {
    ROS_WARN_STREAM("Xbot : no param server setting for publish_tf, using default [" << publish_tf << "][" << name << "].");
  } else {
    if ( publish_tf ) {
      ROS_INFO_STREAM("Xbot : publishing transforms [" << name << "].");
    } else {
      ROS_INFO_STREAM("Xbot : not publishing transforms (see robot_pose_ekf) [" << name << "].");
    }
  }

  if (!nh.getParam("use_imu_heading", use_imu_heading)) {
    ROS_WARN_STREAM("Xbot : no param server setting for use_imu_heading, using default [" << use_imu_heading << "][" << name << "].");
  } else {
    if ( use_imu_heading ) {
      ROS_INFO_STREAM("Xbot : using imu data for heading [" << name << "].");
    } else {
      ROS_INFO_STREAM("Xbot : using encoders for heading (see robot_pose_ekf) [" << name << "].");
    }
  }
